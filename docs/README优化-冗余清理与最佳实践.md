...
> **状态**：历史　｜　日期：2026-09-12　｜　索引：[docs/README-导航与索引.md](README-导航与索引.md) §6　｜　并入：纪律已推广至全库，见 [S12 §7](S12-文档体系SDD诊断与优化方案.md)
...
> 承接文档：[README-导航与索引.md](./README-导航与索引.md)（新）、
> [S9r-README性能体积长口径与公共API明细.md](./S9r-README性能体积长口径与公共API明细.md)（新）。
...
| 新文档 | 承接内容 | 行数 |
|--------|----------|-----:|
| [docs/S9r-README性能体积长口径与公共API明细.md](./S9r-README性能体积长口径与公共API明细.md) | ①②③④：体积表 + `-Oz` 命令 + fast_qr 体积对照 = §1；三口径表 + 明细表 + Roadmap = §2；公共类型表 + 矩阵示例 = §3；脚本分组表 = §4；另附 §5「引用纪律」 | 177 |
| [docs/README-导航与索引.md](./README-导航与索引.md) | ⑤ + 全量清单：§0 读者路径、§1 公共 API 对照、§2 性能体积出处、§3 测试证据链、§5 发布、§6 工程协作、§7 实现系列、§8 全量文档清单（可折叠） | 180 |
...
3. **索引也要有粒度纪律**：把「全量文档清单」放 README 等于把 `docs/` 目录复制一遍；
   正确做法是 README 给**读者路径**（我想… → 去哪），全量清单交给 `docs/README-导航与索引.md`。
