...
| 动作 | 文件 | 内容 |
|------|------|------|
| 修正注释 | `fast_qr_moonbit_test.mbt` | 「包内黑盒」→ 准确的黑盒（包外）说明，补充 `@fast_qr_moonbit` 别名用法 |
...
| `agents.md` → `AGENTS.md` | 对齐官方命名；`agents.md` 改为符号链接保持兼容 |
...
| 优先级 | 事项 |
|:---:|------|
| P0 | 实现 QR 公共 API；届时在 `cmd/main/moon.pkg` 启用 `import { "tryandrun/fast_qr_moonbit" @lib }` 并让 CLI 调用 |
...
- [README.mbt.md](../README.mbt.md) — 代码放置约定与文档索引
