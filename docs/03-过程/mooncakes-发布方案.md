...
**收益（实测）**：`moon test --target wasm-gc` **+1 条 README 文档测试**（计数见 [S10b](../02-证据/S10b-测试覆盖率报告.md) 登记处），
...
- 本文是 **发布/分发** 视角的方案；`docs/01-规格/moonbit-实现布局与文件职责.md` 是**内部布局**视角。
- `docs/01-规格/moonbit-工具链与构建-setup-分析.md` 覆盖工具链安装与 CI 集成，
