...
> **状态**：已失效　｜　日期：2026-09-05　｜　索引：[docs/README.md](../README.md) §7
...
| 对象 | 当时的骨架代码（库注释文件 `fast_qr_moonbit.mbt` 仅含文档注释，CLI 仅 `println` 一行，测试 1 个冒烟用例）——**该文件已于 S11 v4 删除，见 [S11b](../02-证据/S11b-清理落地记录-v3-v5.md) §12** |
...
> （见 [moonbit-实现布局与文件职责.md](../01-规格/moonbit-实现布局与文件职责.md) §四）。
...
> **当前完整实现的产物体积**见
> [S9g 本项目 wasm 产物体积-确认与修正](../02-证据/S9g-本项目wasm产物体积-确认与修正.md)：
...
> 已修正（见 `docs/01-规格/moonbit-实现布局与文件职责.md`）：注释改为准确的黑盒/白盒说明，
...
- [moonbit-工具链与构建-setup-分析.md](../01-规格/moonbit-工具链与构建-setup-分析.md) — 工具链安装与构建系统
- [moonbit-工具链与构建-setup-分析.md](../01-规格/moonbit-工具链与构建-setup-分析.md) — 仓库初始化与 CI 配置
