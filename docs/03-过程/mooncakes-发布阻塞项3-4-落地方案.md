...
> **状态**：现行　｜　日期：2026-09-14　｜　索引：[docs/README.md](../README.md) §5

> 承接 Issue #71 的后续任务：对 PR #72（`docs/03-过程/mooncakes-发布方案.md`）提出的 **4 个阻塞项**中
> 的 **#3（归档面未收敛）** 与 **#4（发布前门禁缺口）** 做**详细分析 + 实测 + 落地**。
>
> 与 [`docs/03-过程/mooncakes-发布方案.md`](mooncakes-发布方案.md) 的关系：**该文是全景评估（要做什么），本文是 #3/#4 的深挖与落地**
...
- **维护者上下文泄漏**：`docs/` 内的文档大量引用 CNB Issue/PR 路径（[`docs/03-过程/mooncakes-发布方案.md`](mooncakes-发布方案.md) §R7），
  对外部读者是断链，且属内部审计记录。

**为什么 #4 是真问题**：

- 官方**不给「按版本」撤回**（`moon publish --help` 无相关选项；`moon deprecate` 只能整模块弃用，见
...
> 口径提示：[`docs/03-过程/mooncakes-发布方案.md`](mooncakes-发布方案.md) §4.3 曾建议「`cmd/main` 保留、其余 `cmd/*` 排除」。
...
[`docs/03-过程/mooncakes-发布方案.md`](mooncakes-发布方案.md) §6-B1 称：开启 `missing_doc`(0074) 后 `lib/**` 有 **13 处**缺口。
...
| 4 | `readme` 迁移到 `README.mbt.md` | 维护者 | [`docs/03-过程/mooncakes-发布方案.md`](mooncakes-发布方案.md) §4.2 已评估，建议 0.1.1 跟进 |
...
- [`docs/03-过程/mooncakes-发布方案.md`](mooncakes-发布方案.md) —— **全景评估**（4 阻塞项 / 风险表 / 落地清单 A–D）。
...
- [`docs/02-证据/S10-测试用例设计与完善roadmap.md`](../02-证据/S10-测试用例设计与完善roadmap.md) —— 门禁 T4/T5/T6/T7 的登记处；
