...
| 原 README 章节 | 处置 | 承接地 |
|----------------|------|--------|
| 「编译为 Wasm」体积表 + `-Oz` 命令 + fast_qr 体积对照 | 下沉 | [S9r §1](./S9r-README性能体积长口径与公共API明细.md) |
| 「性能与体积」三口径表 + 明细表 + Roadmap | 下沉 | [S9r §2](./S9r-README性能体积长口径与公共API明细.md) |
| 「公共类型一览」+「矩阵读写与自建」示例 | 下沉 | [S9r §3](./S9r-README性能体积长口径与公共API明细.md) |
| 「开发与 CI」脚本逐项表 | 下沉 | [S9r §4](./S9r-README性能体积长口径与公共API明细.md) |
| 文档索引长表（`⤷` 系列入口 + 全量篇目） | 下沉 | [README-导航与索引.md](./README-导航与索引.md)（含 §8 全量清单） |
...
| `docs/README-导航与索引.md` 等**仓库内导航面**文档 | 相对链接（该文件自身不在归档内） | 本文件不在归档内，不受归档约束 |
