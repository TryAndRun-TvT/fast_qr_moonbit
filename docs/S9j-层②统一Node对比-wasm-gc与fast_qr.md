...
> **状态**：历史　｜　日期：2026-09-11　｜　索引：[docs/README-导航与索引.md](README-导航与索引.md) §7　｜　并入：现行层②/宿主口径见 [S9p](S9p-宿主调用面性能口径-JS向wasm传参.md)
...
>
> ⚠️ **2026-09-14 口径更新（S9p）**：本文的 MoonBit 侧产物是 `cmd/bench`（**命令形态**，
> 导出面只有 `_start`、输入走 argv），故 A/B 两档都是「代理口径」：A 把 Instance 创建算进每次调用、
> B 把宿主传参抹平。**真实宿主调用形态（一次实例化 + 反复带参调用）见
> [S9p 宿主调用面性能口径](./S9p-宿主调用面性能口径-JS向wasm传参.md)**，其 `fast/ours` =
> **0.407 / 0.620 / 0.756×**，为当前对外引用主口径；本文的 A/B 保留作对照（A 会系统性夸大差距）。
>
