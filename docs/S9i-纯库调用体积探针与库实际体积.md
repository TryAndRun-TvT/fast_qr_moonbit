...
| 产物 | 后端 | raw | `-Oz`（可加载档） |
|------|------|----:|------------------:|
| **`cmd/qr-min`（纯库调用）** | **`wasm-gc`（默认）** | 41294 B | **31470 B**（30.7 KiB） |
...
| **QR 核心管线（库实际体积）** | **`cmd/qr-min`** | **31470** | 仅 build 路径 + 一行 `println`；净增 **+31207 B** |
| + 输出层 | `cmd/main` | 33709 | 终端字符画 + SVG + Shape 共 **+2239 B**（2.2 KB） |
| + 基准外壳 | `cmd/bench` | 36296 | argv/env/迭代/`--dump`/checksum 共 **+4826 B**（4.8 KB） |
