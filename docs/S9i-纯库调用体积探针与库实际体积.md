...
> 且 `cmd/qr-min` 在 P0/P2/P2b 优化后代码规模微调。**当前最新值已更新为**
...
| 产物 | 后端 | raw | `-Oz`（可加载档） |
|------|------|----:|------------------:|
| **`cmd/qr-min`（纯库调用）** | **`wasm-gc`（默认）** | 41294 B → **41427 B** | **31470 B → 31365 B**（30.6 KiB） |
...
| **QR 核心管线（库实际体积）** | **`cmd/qr-min`** | **31365**（首测 31470） | 仅 build 路径 + 一行 `println`；净增 **+31102 B** |
| + 输出层 | `cmd/main` | 33593（首测 33709） | 终端字符画 + SVG + Shape 共 **+2228 B**（2.2 KB） |
| + 基准外壳 | `cmd/bench` | 36174（首测 36296） | argv/env/迭代/`--dump`/checksum 共 **+4809 B**（4.7 KB） |

**一句话**：作为库被嵌入时，本库 QR 核心在默认后端 `wasm-gc` 下约 **30.7 KB**（净增 +31.2 KB），
此前以 `cmd/bench`/`cmd/main` 引用的 33–36 KB 是「命令形态」上限；输出层只占 2.2 KB，
基准外壳占 4.8 KB。**引用本库体积时请用 `cmd/qr-min` 口径并注明后端与优化档。**
...
用命令形态数字对外承诺库体积**系统性偏高**（wasm-gc 侧偏高 ≈1.5–4.8 KB，见 §2 归因）。
