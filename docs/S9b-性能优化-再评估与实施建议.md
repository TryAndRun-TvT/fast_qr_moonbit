...
- **做法**：择优循环记住最优轮**已掩码矩阵**，末尾 `masked_out=apply_mask(base,best)` 直接复用。
...
| 批 1 | T1+T2（免 8+1 copy） | ≈5–6.5ms | ≈0.5–0.65×（1.5–2× 慢） | 快照 diff + checksum |
| 批 2 | T3（减趟）+T4（mask 特化） | ≈4–5ms | ≈0.65–0.8×（1.25–1.5× 慢） | 同上 |
