...
> （矩阵扫描 **≈1.19×**、RS 工作区 **≈1.50×，真实代码 A/B 验证**）；但按占比封顶，矩阵介质约 **≈10–13%**、
> RS 仅 **≈0.8%** 的整 build 量级。
...
| **P3 介质窄化（重定义）** | 目标是**`FixedArray` 化**（≈1.19×），**不是** byte 窄化（≈1.16×，无速度收益） | 先做**真实代码 A/B**（D3 介质级重构，触及 matrix/score/placement/datamasking）；预期 ≈10% 量级、面大 → 需单独立项评审 |
...
RS 工作区是一行就能拿的 **1.50×**（但整 build 仅 ≈0.8%）；矩阵介质虽有 **≈10%** 量级，但要动 D3 介质，
**必须先做真实代码 A/B 再定**。
