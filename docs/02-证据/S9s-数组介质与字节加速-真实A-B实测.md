...
> **语义零差异（147/147 + checksum）**，公共 API 不变。
...
| **P3 介质窄化（重定义）** | 目标是**`FixedArray` 化**（≈1.19×），**不是** byte 窄化（≈1.16×，无速度收益） | 已在 §10 真落两版实测：**V40 宿主 −15%、体积 −5~6%**；待 D3 介质评审 + 8 个白盒测试适配 |
...
- **介质换代（去 `Array` 间接）实测成立**：V40 宿主 **−15%**、体积 **−5~6%**、语义零差异、公共 API 不变。
- **选型**：优先 `FixedArray[Int]`（改动小、无 `Byte` 转换噪声、速度与 Byte 版持平）；
  若**在意宿主侧内存/GC**（多实例、长驻）再上 `FixedArray[Byte]`。
- **前置**：需 D3 介质决策评审（`S4 §3.1 D3` 现明文 `Array[Int]`）+ 更新 8 个白盒测试；
  `.mbti` 不变已实测（`QRCode.data` 未动）。
- 分支 `exp/fixedarray-int` / `exp/fixedarray-byte` 为**实验分支**，**未合并、未推送**，供评审取用。
