...
## 2. 现状盘点：性能关键路径的容器选择（**2026-09-23 换代前基线**）

> ⚠️ 本节记录**换代前**状态（矩阵介质 = `Array[Int]`）；`FixedArray[Int]` 已于 §10 落地（`ea7d7ae`）。
> §4/§5 的「vs 现行」比值与本节同属**换代前**基线，§10 给出换代后的实测。
...
3. **按占比封顶外推（Q3，S9n §3 纪律：不叠加、不重复计数）**：
   - **矩阵介质**受益于 `score`(68%) + `apply_mask`(11%) 的扫描/读写 → 上界 ≈ `79% × (1 − 1/1.19)` ≈ **12.6%**；
     取保守 **≈10%**（`score` 另含 N2/N4 未逐一建模）。**这是量级估算，非整 build 直测**——§10 已真落两版实测
     （V40 宿主 ≈ **−15%**，同向、略高于估算）。
...
| # | 局限 | 影响 |
|:-:|------|------|
| L1 | 矩阵扫描是**代表性内联**，非整函数/整 build A/B；`score` 的 N2/N4 与掩码 `copy` 未逐一建模 | 矩阵 ≈10% 为**量级估算**（§10 真码实测已补正为 ≈−15%） |
...
| 点 | `main`（换代前 `Array[Int]`） | `exp/int` | `exp/byte` | int vs main | byte vs main |
|----|----------------------:|----------:|-----------:|------------:|-------------:|
