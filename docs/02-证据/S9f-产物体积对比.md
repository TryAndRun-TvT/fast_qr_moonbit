...
> **状态**：历史　｜　日期：2026-09-10　｜　索引：[docs/README.md](../README.md) §7　｜　并入：体积现行口径见 [S9i](S9i-纯库调用体积探针与库实际体积.md)
...
- issue #50（本任务来源）；[S9e 方案](S9e-性能测试统一Node调用.md)（性能侧同尺子）、
  ...
  [wasm-编译与运行-结果分析](../03-过程/wasm-编译与运行-结果分析.md)（早期三后端产物体积对照）。
...
- 性能侧：[S9e 实现记录](S9e-性能测试统一Node调用.md)（同尺子性能）、
  [S9c 实现记录](S9c-性能测试与fast_qr-wasm对比.md)（层②构建链路）。
- 体积前史：[wasm-编译与运行-结果分析](../03-过程/wasm-编译与运行-结果分析.md) §4/§5（早期三后端产物体积对照）。
