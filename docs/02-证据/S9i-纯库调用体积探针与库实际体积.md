...
> fast_qr 参考侧产物逐字节未变（裸探针 `-Oz` 45687 B）。本节 §0 数字随后更新为**再测值**；
> 末次明细与结论见 [S9r](S9r-README性能体积长口径与公共API明细.md) §1，§3 各表保留 09-11 首测原值。
>
> 🟢 **2026-09-23 介质换代落地（`Array[Int]` → `FixedArray[Int]`，提交 `ea7d7ae`）**：
> `cmd/qr-min` raw **39869 B** / `-Oz` **29649 B**（较 31629 **−6.3%**）；`cmd/main` `-Oz` **31852 B**（−5.9%）；
> `cmd/bench` `-Oz` **37731 B**（−5.0%）。净增 **+29386 B**（hello 地板仍 263 B）；fast_qr 裸探针仍 45687 B
> ⇒ **对称锚点 0.65×**（原 0.69×）。原因/实测见 [S9s §10](S9s-数组介质与字节加速-真实A-B实测.md)；
> **下列 §0 表已同步为换代后值**（`wasm`(WASI) 行的历史值未重测，仅兜底记录）。
...
| 产物 | 后端 | raw | `-Oz`（可加载档） |
|------|------|----:|------------------:|
| **`cmd/qr-min`（纯库调用）** | **`wasm-gc`（默认）** | 41294 B → **39869 B** | **31470 B → 29649 B**（29.0 KiB） |
...
| **QR 核心管线（库实际体积）** | **`cmd/qr-min`** | **29649**（09-12 为 31365） | 仅 build 路径 + 一行 `println`；净增 **+29386 B** |
| + 输出层 | `cmd/main` | 31852（09-12 为 33593） | 终端字符画 + SVG + Shape 共 **+2203 B**（2.2 KB） |
| + 基准外壳 | `cmd/bench` | 37731（09-12 为 36174） | argv/env/迭代/`--dump`/checksum 共 **+8082 B**（7.9 KB） |

**一句话**：作为库被嵌入时，本库 QR 核心在默认后端 `wasm-gc` 下约 **29.0 KB**（净增 **+29386 B**），
此前以 `cmd/bench`/`cmd/main` 引用的 31–37 KB 是「命令形态」上限；输出层只占 2.2 KB，
