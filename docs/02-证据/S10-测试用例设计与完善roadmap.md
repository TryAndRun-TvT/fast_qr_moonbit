...
> **状态**：现行　｜　日期：2026-09-14　｜　索引：[docs/README.md](../README.md) §3
...
> **T6-c**（`docs-link-check.sh` 死链检查；原进 push CI，现本地/按需执行）。
...
| 项 | 内容 | 验收 |
|----|------|------|
| T5-a ✅**（v4 已落）** | `scripts/coverage.sh`（封装 `moon coverage analyze`）→ `docs/02-证据/S10b-测试覆盖率报告.md`；单列 `lib/**` 合计 | ✅ 未覆盖 87 → 68（lib 44 → 25）；**不承诺阈值** |
...
| 文档 | 关系 |
|------|------|
| [moonbit-实现布局与文件职责.md](../01-规格/moonbit-实现布局与文件职责.md) | **上游**：§5「测试规划」定义三载体分工；本文细化到用例级 |
| [S6-端到端对齐与公共API.md](../01-规格/S6-端到端对齐与公共API.md) | 提供 `m1`/`s6` 快照的生成方法（本文 T0/T1 沿用同一生成器模式） |
...
| [S7-输出层to_str与SVG.md](../01-规格/S7-输出层to_str与SVG.md) | 输出层测试（`s7_*`）的语义基准 |
...
18b. [x] 文档互链死链检查（T6-c，`scripts/docs-link-check.sh`；原进 push CI，**现本地/按需执行**）
...
| T5-a | `scripts/coverage.sh` ＋ `docs/02-证据/S10b-测试覆盖率报告.md` | 封装 `moon coverage analyze`，单列 `lib/**` 合计；**不设阈值** | 未覆盖 87 → 68（lib 44 → 25） |
...
| T6-c | `scripts/docs-link-check.sh`（原进 push CI，现本地/按需执行） | 受版本控制 Markdown 的**相对链接**存在性检查；跳过外链/锚点；**忽略代码块** | 零死链（文件数/条数以实跑为准）；植入假死链 → 退出码 1 |
