...
- `preferred_target` 已改为 **`wasm-gc`**（依据见
  [wasm-编译与运行-结果分析.md](../03-过程/wasm-编译与运行-结果分析.md)）。
...
- **`fast_qr_moonbit.mbt` 已删除**（S11 v4，2026-09-14）：该文件后来被刷新为纯注释入口，因「零可执行行、
  ...
  [S11b](../02-证据/S11b-清理落地记录-v3-v5.md) §12。上方初始结构树为**初始化当时**的快照。
...
> （见 [wasm-编译与运行-结果分析.md](../03-过程/wasm-编译与运行-结果分析.md) §6.1）。
...
- `.codebuddy/` 与 `AGENTS.md` 最初来源于 `chathub-server`，其原内容为 wrangler/Cloudflare 相关约定，与本 MoonBit 项目无关。
  **已于 2026-09-05 重写**，替换为 MoonBit 项目约定（包/测试文件放置、依赖声明、后端与构建、收尾检查），
  并去除对不存在文件的引用；同时重命名为官方 `AGENTS.md`。详见 [moonbit-实现布局与文件职责.md](moonbit-实现布局与文件职责.md)。
