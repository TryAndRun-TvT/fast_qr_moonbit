...
> 依据 [项目基础框架-详细分析](../03-过程/项目基础框架-详细分析.md)、
> [moonbit-重写-roadmap-详细分析](../03-过程/moonbit-重写-roadmap-详细分析.md) 与
...
> 计数不写死（会漂移）：登记处 = [S10b](../02-证据/S10b-测试覆盖率报告.md) 顶部 `<!-- test-count: N -->`。
> 本文的布局/职责结论仍然有效，但 §3 的「状态」列与 §6 的落地描述已按**当前真实状态**刷新
> （2026-09-14，[S11b](../02-证据/S11b-清理落地记录-v3-v5.md) §13「过期注释/口径巡检」）。
>
...
>
> **v4 更新（2026-09-14，[S11b §12](../02-证据/S11b-清理落地记录-v3-v5.md)）**：原 `lib/fast_qr_moonbit.mbt`
...
| `qr.mbt` | QRCode 容器 / 错误 / 读写访问器（公共 API 总览见 [S11b §12](../02-证据/S11b-清理落地记录-v3-v5.md)） | `src/qr.rs` | B8 | 已实现 |
...
- [moonbit-重写-roadmap-详细分析.md](../03-过程/moonbit-重写-roadmap-详细分析.md) — roadmap、B1-B11 与里程碑
- [项目基础框架-详细分析.md](../03-过程/项目基础框架-详细分析.md) — 目标包架构、模块映射、移植语义
- [core-仓库布局参考与目标架构.md](../03-过程/core-仓库布局参考与目标架构.md) — internal/ 目标树依据
...
> 此项修正了 `docs/03-过程/wasm-编译与运行-结果分析.md` §6.3 原先「必须补 import」的表述。
...
| 修正 | `docs/03-过程/wasm-编译与运行-结果分析.md` | §6.2 / §6.3 补充实证结论；§7.2 待办表加状态列并标记已完成项 |
| 更新 | `docs/01-规格/moonbit-工具链与构建-setup-分析.md` | 注意事项同步为「agents.md 已适配」 |
| 新增 | `docs/01-规格/moonbit-实现布局与文件职责.md` | 本文档 |
| 新增 | `README.md` -> `README.mbt.md` 符号链接 | 官方布局含此链接；git 以 `mode 120000` 跟踪 |
| 新增 | `docs/01-规格/moonbit-项目目录设置-最佳实践.md` | 依据官方文档整理的目录设置最佳实践 |
...
| ~~P0~~ | ~~测试从 `assert_true(true)` 升级为真实断言~~ → **已完成**（S10 收口，用例数见 [S10b](../02-证据/S10b-测试覆盖率报告.md) 登记处） |
...
- [wasm-编译与运行-结果分析.md](../03-过程/wasm-编译与运行-结果分析.md) — 后端选型与产物分析
...
[README优化-冗余清理与最佳实践.md](../04-元/README优化-冗余清理与最佳实践.md) §3.1），
...
#### v5：README 定位收缩为「落地页 + 索引」（2026-09-14）

在方向回正（v4）之后，README 仍**过长**（485 行 / 35 KB）：体积表、性能明细表、Roadmap 编号、
公共类型表、脚本逐项表都留在正文里，与 `docs/S9*` 重复。v5 把**明细整体下沉**：

| 原 README 章节 | 处置 | 承接地 |
|----------------|------|--------|
| 「编译为 Wasm」体积表 + `-Oz` 命令 + fast_qr 体积对照 | 下沉 | [S9r §1](../02-证据/S9r-README性能体积长口径与公共API明细.md) |
| 「性能与体积」三口径表 + 明细表 + Roadmap | 下沉 | [S9r §2](../02-证据/S9r-README性能体积长口径与公共API明细.md) |
| 「公共类型一览」+「矩阵读写与自建」示例 | 下沉 | [S9r §3](../02-证据/S9r-README性能体积长口径与公共API明细.md) |
| 「开发与 CI」脚本逐项表 | 下沉 | [S9r §4](../02-证据/S9r-README性能体积长口径与公共API明细.md) |
| 文档索引长表（`⤷` 系列入口 + 全量篇目） | 下沉 | [README-导航与索引.md](../README.md)（含 §8 全量清单） |

README 保留：头部（截图/徽章/状态/元数据表）、功能特性、能力与局限、快速开始（`mbt check` 唯一示例）、
构建与运行（命令 + 本地门禁一行）、性能与体积（**量级 + 出处**）、测试（**分层 + 证据链 + 维护入口**）、
文档索引（**8 行「我想… → 去哪」表**）、项目结构、工程约定与贡献、License。

**度量**：485 → 280 行（−42%）、35155 → 16983 字节（−52%）；
`bash scripts/docs-link-check.sh` 零死链（62 个 Markdown / 595 条相对链接）。
**纪律已写入 [AGENTS.md](../../AGENTS.md) §四**：新增内容先判「读者第一屏需要」还是「证据」，后者进 `docs/`。

#### v6：发布面可达性闭环 —— README 的链接形态与「分发面」对齐（2026-09-14）

v5 把 README 收缩为「落地页 + 索引」，**下沉本身是对的**，但**指向下沉内容的链接形态选错了**：
README 用 `./docs/**` **相对链接**，而发布归档（`.moonignore`）**排除 `/docs/` 与 `/AGENTS.md`**，
于是 mooncakes 落地页把这些链接重写为 `assets.mooncakes.io/source/.../docs/...` → **成片 404**。

**实测口径**（发布归档可达性，逐路径请求）：

| 路径 | 归档成员 | HTTP |
|------|:--------:|:----:|
| `README.md` / `LICENSE` / `moon.mod` / `cmd/main/main.mbt` | ✅ | **200** |
| `docs/**` / `AGENTS.md` | ❌（被 `.moonignore` 排除） | **404** |

**结论（v6 纪律）**：README 的链接形态**按目标是否分发**二分——

| 目标 | 形态 | 理由 |
|------|------|------|
| `README.md` / `LICENSE` / `cmd/main/**`（**归档内**） | 相对链接 | 归档读者可点；仓库内亦自洽 |
| `docs/**` / `AGENTS.md`（**归档外**） | **仓库绝对链接**（`https://cnb.cool/.../-/blob/main/<path>`） | 归档读者可点（CNB blob 实测 200）；且锚点在**渲染页**可滚动定位 |
| `docs/README.md` 等**仓库内导航面**文档 | 相对链接（该文件自身不在归档内） | 本文件不在归档内，不受归档约束 |

**门禁落点**：新增 `bash scripts/publish-check.sh` **⑤ 发布面链接可达性**（纯离线）——
断言「归档内 Markdown 的相对链接**全部**指向归档成员」，并在 AGENTS.md §四 固化为纪律。
与 `docs-link-check.sh`（查仓库内目标是否存在、跳过锚点）**判据不同、互补**。

**度量**：README 中指向归档外的相对链接 **47 → 0**；README 280 → 300 行（+「链接约定」小节）；
`moon test` 仍 147/147（示例未动）。详见
