...
7. **目录按读者意图分，不按特性/模块分**（2026-09-18，S12b §18）：特性是**代码**的切法
   （`lib/internal/<模块>/` 各带 `moon.pkg`），文档的切法是**读者问题**。
   **新增子目录须过 R1–R5**（读者意图不同 / ≥3 篇且会增长 / 独立生命周期 /
   能接入索引与门禁（③/③b/③d - 含更新 §0.5 地图）/ 域首页不得叫 `README.md`）；
   **仅「文件名同族」或「以后可能会多」不构成建目录理由**（Simplicity Gate）。
8. **归类固定 4 类 + 移植参考**（D2 目录分类，2026-09-18）：新篇按**读者意图**放，勿再平铺顶层——
...
- **漂移门禁**（三条，均接入 `scripts/gates.sh`）：
  - `bash scripts/docs-consistency.sh`：八项断言（① 状态 / ② 计数 / ②b 版本 /
    ③ 索引正向（含**摘要区按名链接**）/ ③b 索引反查 / **③d 索引链接内容（标签非空、锚点非占位）** /
    ③c 无子目录 `README.md` / ④ 规模）
    + ⑤ 日期漂移**提示**（非阻断）；
